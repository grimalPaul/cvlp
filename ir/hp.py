...
class BM25Objective(Objective):
    ...
    def __call__(self, trial):
        ...
        self.es_client.indices.close(index = self.es_index_name)
        self.es_client.indices.put_settings(body = settings, index = self.es_index_name)
        self.es_client.indices.open(index = self.es_index_name)
