...
COMMONS_PATH = Path("/scratch_global/stage_pgrimal/data/miniViQuAE/data/dataset/miniCommons")
#IMAGE_PATH = Path(os.environ.get("VIQUAE_IMAGES_PATH", COMMONS_PATH))
IMAGE_PATH  = Path('/scratch_global/stage_pgrimal/data/miniViQuAE/data/dataset/miniCommons')
...
def load_faces(image, root_face_path, max_n_faces=None):
    ...
    while (max_n_faces is None) or (i <= max_n_faces):
        face_path = (root_face_path /
                     f"{image_stem}_{i}").with_suffix(image_suffix)
        if face_path.exists():
...
def map_kilt_triviaqa():
    ...
    for k in ['train', 'validation', 'test']:
        ...
        kilt_tasks[k + '_triviaqa'] = kilt_tasks[k +
                                                 '_triviaqa'].filter(lambda x: x['id'] in triviaqa_map)
        kilt_tasks[k + '_triviaqa'] = kilt_tasks[k + '_triviaqa'].map(
            add_missing_data,
            fn_kwargs=dict(
                trivia_qa_subset=trivia_qa[k], triviaqa_map=triviaqa_map)
        )
...
def make_mapping_dataset(dataset_path, key, save_name, **kwargs):
    ...
    mapping = {}
    dataset.map(make_mapping, input_columns=key, with_indices=True,
                fn_kwargs=dict(mapping=mapping, **kwargs))
...
def make_passages(paragraphs, method=None, preprocessing_method=None, preprocessing_kwargs={}, **kwargs):
    ...
    """
    paragraphs = paragraphs_preprocess(
        paragraphs, method=preprocessing_method, **preprocessing_kwargs)
    methods = {
        None: lambda paragraphs: paragraphs,
...
def make_passage_item(item, index, passage_dict, prepend_title=False, **kwargs):
    ...
    item['passage_index'] = list(
        range(total_passages, total_passages+len(passages)))
...
def make_passage_dataset(input_path, output_path, sentencizer=False, **kwargs):
    ...
    dataset = dataset.map(make_passage_item, with_indices=True, fn_kwargs=dict(
        passage_dict=passage_dict, **kwargs))
